...
def test_gli_offsets_del_promemoria_hanno_la_precedenza(make_reminder, app_settings):
    reminder = make_reminder(60, alert_offsets=[7, 2])
    assert alerts.effective_offsets(reminder, app_settings) == [7, 2]


def test_senza_offsets_propri_si_usano_quelli_globali(make_reminder, app_settings):
    reminder = make_reminder(60)
    assert alerts.effective_offsets(reminder, app_settings) == [30, 15, 7, 3, 1, 0]


def test_offsets_ripuliti_da_duplicati_e_negativi(make_reminder, app_settings):
    reminder = make_reminder(60, alert_offsets=[7, 7, -3, 1])
    assert alerts.effective_offsets(reminder, app_settings) == [7, 1]


def test_un_preavviso_per_ogni_offset_futuro(local_db, make_reminder, app_settings):
    """Scadenza fra 60 giorni: tutti i preavvisi sono ancora davanti."""
    reminder = make_reminder(60, alert_offsets=[30, 7, 0])
...
def test_preavvisi_gia_passati_non_vengono_sparati_tutti_insieme(local_db, make_reminder, app_settings):
    """Scadenza inserita oggi a 10 giorni: i preavvisi a 30 e 15 sono nel
    passato e non devono generare avvisi retroattivi."""
    reminder = make_reminder(10, alert_offsets=[30, 15, 7, 3, 1, 0])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)
...
def test_orario_di_invio_rispetta_il_fuso_configurato(local_db, make_reminder, app_settings):
    ...
    reminder = make_reminder(60, alert_offsets=[30])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)
...
def test_scadenza_gia_scaduta_genera_un_avviso_immediato(local_db, make_reminder, app_settings):
    ...
    reminder = make_reminder(-5, alert_offsets=[30, 7, 0])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)
...
def test_nessun_recupero_se_un_preavviso_cade_gia_oggi(local_db, make_reminder, app_settings):
    ...
    reminder = make_reminder(3, alert_offsets=[30, 3, 0])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)
...
def test_nessun_recupero_fuori_dalla_finestra_di_preavviso(local_db, make_reminder, app_settings):
    """Scadenza lontana: nessun avviso deve partire oggi."""
    reminder = make_reminder(60, alert_offsets=[30, 7])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)
...
def test_solleciti_dopo_la_scadenza(local_db, make_reminder, app_settings):
    app_settings.overdue_repeat_days = 3
    app_settings.overdue_max_reminders = 4
    reminder = make_reminder(30, alert_offsets=[7])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)

    dopo = [g for g in _alert_days(local_db) if g > reminder.due_date]
    assert dopo == [reminder.due_date + timedelta(days=n * 3) for n in range(1, 5)]


def test_solleciti_disattivabili(local_db, make_reminder, app_settings):
    app_settings.overdue_repeat_days = 0
    reminder = make_reminder(30, alert_offsets=[7])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)

    assert all(g <= reminder.due_date for g in _alert_days(local_db))


# ----------------------------------------------------------------- idempotenza


def test_risincronizzare_non_duplica_gli_avvisi(local_db, make_reminder, app_settings):
    reminder = make_reminder(60, alert_offsets=[30, 7, 0])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)
...
def test_un_avviso_gia_inviato_non_viene_rigenerato(local_db, make_reminder, app_settings):
    reminder = make_reminder(60, alert_offsets=[30, 7, 0])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)
...
def test_spostare_la_data_ricalcola_i_preavvisi_pendenti(shared_db, local_db, make_reminder, app_settings):
    ...
    reminder.due_date = date.today() + timedelta(days=90)
    shared_db.commit()
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)
...
def test_scadenza_evasa_non_ha_piu_avvisi_pendenti(shared_db, local_db, make_reminder, app_settings):
    reminder = make_reminder(60, alert_offsets=[30, 7])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)
...
def test_sync_all_copre_solo_le_scadenze_aperte(shared_db, local_db, make_reminder):
    aperta = make_reminder(60, title="Aperta", alert_offsets=[30])
    chiusa = make_reminder(60, title="Chiusa", alert_offsets=[30])
    chiusa.status = ReminderStatus.DONE
    shared_db.commit()

    alerts.sync_all(shared_db, local_db)

    assert {n.reminder_id for n in _pending(local_db)} == {aperta.id}


def test_sync_all_rimuove_avvisi_di_scadenze_sparite(shared_db, local_db, make_reminder, app_settings):
    """Una scadenza evasa o eliminata da un'altra postazione non deve lasciare
    avvisi pendenti su questa."""
    reminder = make_reminder(60, alert_offsets=[30])
    alerts.sync_reminder_notifications(local_db, reminder, app_settings)

    shared_db.delete(reminder)
    shared_db.commit()
    alerts.sync_all(shared_db, local_db)
...
def test_due_notifications_solo_quelle_arrivate_a_scadenza(local_db, make_reminder, app_settings):
